...
const RestaurantMenu = () => {
  ...
  return (
    <div>
      <Toaster />
      <div className="banner text-white h-36 flex flex-col flex-wrap justify-evenly items-center bg-slate-900 ">
        <span className=" text-center text-2xl">{bannerData?.name}</span>
        <div className="flex border w-full  justify-evenly bg-slate-800 p-2">
          <span>{bannerData?.areaName}</span>
          <span>|</span>
          
          <span>{bannerData?.avgRating}</span>
          <span>|</span>
         
          <span>{bannerData?.sla}</span>

        </div>
        {/* <div>
          <span>{Object.values(menuChar).name}</span>
        </div>
        <div className="flex flex-col text-white font-semibold text-2xl">
          <span>{restaurant?.name}</span>
          <span className="text-base font-thin">{restaurant?.areaName}</span>
